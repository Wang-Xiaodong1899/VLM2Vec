...
def main():
    ...
    if getattr(model_args, "lora", False):
        logger.info("model_args.lora=True, but train_dec.py trains only PostQwen2VLLayers; disabling LoRA.")
        model_args.lora = False
    if hasattr(model_args, "lora_init_merge") and not getattr(model_args, "lora_init_merge"):
        logger.info("model_args.lora_init_merge=False, but train_dec.py does not use LoRA; forcing merge to remove adapters.")
        model_args.lora_init_merge = True

    model = MMEBModelPostQwen2VLLayers.build(model_args)

    for p in model.parameters():
        p.requires_grad = False
    if not hasattr(model, "post_decoder"):
        raise AttributeError("MMEBModelPostQwen2VLLayers must have attribute 'post_decoder' to train.")
    for p in model.post_decoder.parameters():
        p.requires_grad = True

    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f"Trainable params (PostQwen2VLLayers only): {trainable_params:,} / {total_params:,}")
